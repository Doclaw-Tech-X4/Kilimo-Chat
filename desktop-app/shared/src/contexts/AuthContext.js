...
const INITIAL_SESSION = { isAuthenticated: false, user: null, token: null };
...
const authReducer = (state, action) => {
  switch (action.type) {
    ...
    case AUTH_ACTIONS.LOGOUT:
      return { ...state, ...INITIAL_SESSION, isLoading: false, error: null };
...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (phone_number, password) => {
    try {
      ...
      const message = response.detail || 'Login failed';
      dispatch({ type: AUTH_ACTIONS.LOGIN_FAILURE, payload: message });
      return { success: false, error: message };
    } catch (error) {
      const message = error?.message || 'Unable to complete login';
      dispatch({ type: AUTH_ACTIONS.LOGIN_FAILURE, payload: message });
      return { success: false, error: message };
    }
  };
  ...
  const loginWithToken = async (token) => {
    ...
    try {
      ...
      localStorage.removeItem('token');
      dispatch({ type: AUTH_ACTIONS.LOGIN_FAILURE, payload: 'Session expired' });
      return { success: false, error: 'Session expired' };
    } catch (error) {
      localStorage.removeItem('token');
      dispatch({ type: AUTH_ACTIONS.LOGIN_FAILURE, payload: 'Failed to load user' });
  ...
  const register = async (userData) => {
    try {
      ...
      const message = response.detail || 'Registration failed';
  ...
  const verifyEmail = async (user_id, code) => {
    try {
      ...
      const message = response.detail || 'Verification failed';
      dispatch({ type: AUTH_ACTIONS.VERIFY_EMAIL_FAILURE, payload: message });
      return { success: false, error: message };
    } catch (error) {
